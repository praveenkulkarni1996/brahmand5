...
def test_generate_strong_password_no_ambiguous_chars():
    password = crypto.generate_strong_password(length=100) # Long password to increase chance of ambiguous chars
...
def test_generate_strong_password_value_error_too_short():
    with pytest.raises(ValueError, match="Password length must be at least 4 to ensure complexity."):
...
def test_generate_command_error_too_short():
    ...
    assert "Password length must be at least 4 to ensure complexity." in str(result.exception)
